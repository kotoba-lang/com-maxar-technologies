(ns maxar-technologies.main-test
  "Contract + behavioral test for the maxar-technologies-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "maxar_technologies-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'maxar-technologies.main-test)]
