(ns maxar-technologies.main
  "Kotodama WASM entrypoint for the Maxar_technologies clean-room actor (L4) — Clojure port.
  ...
  facts (`maxar_technologies.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "maxar_technologies")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"maxar_technologies.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "maxar_technologies-compat" :tier tier :entities entities} 200])
...
(defn start! [] :maxar_technologies-compat/ready)
